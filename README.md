...
- **Don't ask before AI edits** (Settings → AI → Confirmations, off by default): when enabled, Obby does not ask before the AI replaces a note with a much shorter version or makes a large change such as moving, renaming or editing several notes. Deleting a note or folder always asks for confirmation. Every AI edit keeps its Undo button, and the read-before-rewrite requirement, the protection against overwriting a note that changed after it was read, and the notes-folder sandbox always apply.
